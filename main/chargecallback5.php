...
if(empty($postDataArr)){
	$file = "/tmp/pay5.log";
	@file_put_contents($file, "{$curtime}:回调参数错误\n",FILE_APPEND);
...
if($postDataArr['rtnCode']==="0000"){
	$postDataArr['paySerialNo'] = str_replace($postDataArr['merchantNo'],'',$postDataArr['paySerialNo']);
	$attachArr = explode("_", $postDataArr['paySerialNo']);
	$id = (int)$attachArr[1];
