<?php
include_once 'IELPMConfig.php';
include_once 'IELPMCommon.php';
...
function sign(&$params) {
	...
	// 签名证书路径
	$cert_path = IELPM_PRIVATE_CERT_PATH;
...
function verify($params) {
	global $log;
	// 公钥
	$public_key = getPublicKey ( IELPM_PUBLIC_CERT_PATH );	
	// 签名串
...
function getPrivateKey($cert_path) {
	...
	openssl_pkcs12_read ( $pkcs12, $certs, IELPM_PRIVATE_CERT_PWD );
	return $certs ['pkey'];
}
 ...
 * @return unknown
 */
function encryptData($data, $cert_path = IELPM_PUBLIC_CERT_PATH) {
 ...
 * @return unknown
 */
function decryptData($data, $cert_path=IELPM_PRIVATE_CERT_PATH) {
