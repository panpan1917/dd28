 ...
 function show($mode=1,$style='',$nowindex_style='')
 {
  switch ($mode)
  {
   ...
   case '5':
    ...
    $this->first_page='首页';
    $this->last_page='尾页';
    return "共".$this->total."条记录&nbsp;&nbsp;当前".$this->nowindex."/".$this->totalpage."页 ".$this->first_page($style,$nowindex_style)." ".$this->pre_page($style,$nowindex_style)." ".$this->nowbar($style,$nowindex_style)." ".$this->next_page($style,$nowindex_style)." ".$this->last_page($style,$nowindex_style);
    break;
   case '6':
    $this->next_page='>';
    $this->pre_page='<';
