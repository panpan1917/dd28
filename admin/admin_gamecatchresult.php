...
<body>
	<div class="bodytitle">
		...
		<div class="bodytitletxt">游戏采集监测</div>
		<div class="bodytitletxt2">
			<a href="javascript:RemoveCatchResult(1);">删除一天前结果</a>
		</div>
	</div>
	<div>
		<table class="tbtitle" width="99%" cellspacing="1" cellpadding="0" border="0" align="center" style="BACKGROUND: #cad9ea;" >
			<tr bgcolor="#FFFFFF">
				<td colspan="2">说明:针对开奖执行超时的可设为重开奖</td>
			</tr>
			<tr bgcolor="#FFFFFF">
				<td>
					类型
					<select id = "sltKindType">
						<option value="">所有</option>
						<option value="gamebj">北京快乐8</option>
						<option value="gamepk">北京PK拾</option>
						<option value="gamehg">韩国快乐8</option> 
						<option value="gamecan">加拿大快乐8</option> 
					</select>
					期号
				   <input id="txtNo" type="text" style="width:100px" />
				   <input id="cbxTime" type="checkbox"/>采集时间
			  ...
			  <td width="180">
				  <select id = "sltOrder">
						<option value="id">采集时间</option>
						<option value="gametype">游戏种类</option>
						<option value="gameno">期号</option>
					</select>
					&nbsp;
...
<script type= "text/javascript" language ="javascript">
	$(window.parent.document).attr("title","游戏采集监测");
    ...
    {   
    	...
    	$("#PageInfo").html('');     
        var data = "action=get_catchresult";
        var GameKind = $("#sltKindType").val();
        var No = $("#txtNo").val();
        ...
        data += "&gamekind=" + GameKind + "&no=" + No + "&PageSize=" + PageSize;
    ...
    function ReOpen(id)
    {
		var data = "action=catchresult_reopen&id="+ id;
		SendAjax(data);
		GetCatchResult();
    }
    function RemoveCatchResult(day)
    {
		var data = "action=catchresult_remove&day="+ day;
		SendAjax(data);
		GetCatchResult();
    }
    //****************************************************************************************
