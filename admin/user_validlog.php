...
<body>
	...
	<div>
		<table class="tbtitle" width="99%" cellspacing="1" cellpadding="0" border="0" align="center" style="BACKGROUND: #cad9ea;" >				
			<tr bgcolor="#FFFFFF">
				<td><label id="lblUserIdx"></label>短信/邮件记录
			...
			<tr bgcolor="#FFFFFF">
			  ...
			  <td width="180">
				  <select id = "sltOrder">
						...
						<option value="code_type">类型</option>
...
<script type= "text/javascript" language ="javascript">
    ...
    {   
    	...
    	$("#PageInfo").html('');     
        var data = "action=get_validlog";
        var useridx = $("#txtUserIdx").val();
        var codetype = $("#sltType").val();
        ...
        data += "&type=" + codetype + "&state=" + state + "&PageSize=" + PageSize;
        if(useridx != "")
        {
            if(isNaN(useridx))
            {
	...
	function DataSuccess(json)
	{
		...
		$.each(json,function(i,item){
			...
			{
				if(InfoType == "get_validlog")
                {
                    tbody += "<tr bgcolor='#FFFFFF'>" +
                            "<td>" + item.UserID +"</td>" +
                            "<td>" + item.NickName +"</td>" +
                            "<td>" + item.OprTime +"</td>" +
                            "<td>" + item.ValidType +"</td>" +
                            "<td>" + item.Account + "</td>" +
                            "<td>" + item.State + "</td>" +
							"<td>" + item.Content + "</td>" +
							"<td>" + item.ErrMsg + "</td>" +  
